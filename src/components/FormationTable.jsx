...
export default function FormationTable({ label, groupedData = null, isAdmin, type }) {
        ...
        const fetchData = async () => {
            try {
                ...
                let formattedRows = Object.entries(formationData).map(([group, data]) => {
                    ...
                    const totalDamage = Object.values(formationData).reduce((sum, item) => (!isNaN(item.avgDamage) && item.avgDamage > 0) ? sum + item.avgDamage * (item.count || 0) : sum, 0);

                    const share = totalDamage > 0 && damage > 0 ? (damage * count) / totalDamage : 0;
                    const troops = parseFloat((archerVal * share).toFixed(2));
                    ...
                    return {
                        ...
                        t10: MathRound((troops * (settingData.t10 || 0) / 100) / 1000),
                        t9: MathRound((troops * (settingData.t9 || 0) / 100) / 1000),
                        t8: MathRound((troops * (settingData.t8 || 0) / 100) / 1000),
                        t7: MathRound((troops * (settingData.t7 || 0) / 100) / 1000),
                        t6: MathRound((troops * (settingData.t6 || 0) / 100) / 1000),
                        marchSize: 0, // will fill after
                ...
                setIsEdited(false); // fetched data should NOT trigger upload
            } catch (err) {
    ...
    const handleChange = (idx, value) => {
        ...
        setRows(updated);
        setIsEdited(true); // mark as manually edited
    };
    ...
    return (
        <Box sx={{ mt: 4 }}>
            <Box sx={{ mb: 2, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <Button variant="outlined" size="small" onClick={() => setRefreshKey(prev => prev + 1)}>
                    Reload Data
                </Button>
            </Box>
